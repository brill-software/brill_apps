/**
 * Retrives rows from the employee table. 
 * Supports paging and sorting and can thus handle a table with millions of rows. 
 * 
 * Filter:
 * {
 *      offset: number
...
function processRequest(filter, db) {
    ...
    if (filterObj.search_text) {
        ...
        whereClause += " match (first_name,last_name, department) against ('" + 
                    filterObj.search_text + "*' in boolean mode)"
    }

    var countQuery = "select count(*) count from employee" + whereClause;
