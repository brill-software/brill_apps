...
function processRequest(filter, db) {
    ...
    var query = "select user_id, username, first_name, last_name, email, last_login, repository, workspace, case when length(password) > 30 then '******' else password end password, permissions, hidden_apps, changePassword from brill_cms_user where deleted != 'Y'";
